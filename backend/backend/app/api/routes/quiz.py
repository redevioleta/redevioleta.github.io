...
from ...schemas.schemas import QuizPerguntaOut, QuizRespostaIn, QuizResultadoOut

router = APIRouter(prefix="/quiz", tags=["Quiz de Autoavaliação"])

@router.get("/perguntas", response_model=List[QuizPerguntaOut])
def listar_perguntas(db: Session = Depends(get_db)) -> List[Any]:
    return db.query(Quiz).all()


@router.post("/resultado", response_model=QuizResultadoOut)
def calcular_resultado(dados: QuizRespostaIn) -> Any:
    pontuacao: int = sum(dados.respostas)
